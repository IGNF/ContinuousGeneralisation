...
\subsection{Growing based on buffering}
...
buffer is more than $\alpha d_G$, where $d_G$ is the growing distance, away 
...
we must have $\alpha d_G \geq \sqrt{2}$. 
We set $\alpha d_G = 1.5$ so that a square joining is applied when an angle is 
smaller than $83.6 \degree$.

We use value $1.5$ so that we can keep
...
\begin{figure}[tb]
	...
	\caption{Two typical ways of buffering a polygon.}
...
\subsection{Simplifying merged buildings}
...
(S_\mathrm{g}-S_\mathrm{s})$ mm so that it can survive when users zoom out from 
the start map to the goal map.
In order to eliminate small buildings, we define our growing distance as
\begin{equation}
\label{eq:d_G}
	d_\mathrm{G}=\frac{\lambda}{2}\sqrt{a} (S_\mathrm{g}-S_\mathrm{s}) \text{ 
	mm} \quad 0 < \lambda <1,
\end{equation}
where variable $\lambda$ is a growing factor. A larger $\lambda$ results in  
...
$\epsilon$ mm on map.
The real threshold distance is
\begin{equation}
\label{eq:d_epsilon}
...
Studio 
2015) and ArcObjects SDK 10.4.1. 
...
At some point, we should remove the requirement of keeping right angles. Right 
angles may not be important anymore when we deal with settlement boundaries.
