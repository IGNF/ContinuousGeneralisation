...
\[
d_{\mathrm{G}, t} = t \cdot d_\mathrm{G}.
\]
...
Following \textcite{Basaraner2008,Stoter2009}, we set the separation threshold 
$\epsilon= 0.2~\text{mm}$ on map. The separation threshold at time $t$ is
\[
d_{\epsilon, t} = t \cdot \epsilon (S_\mathrm{g}-S_\mathrm{s}).
\]
...
Note that the two bridges in \fig\ref{fig:GrowAndBridge} at time $t=1$ have the 
same widths.
We know where we should add bridges at time $t=0$. 
We grow the bridges together with the buildings.
We do not show the bridges until the two buildings are close enough 
which means that, in our case, 
the distance of two buildings is smaller than $d_{\epsilon,t}$. 
