...
Our aggregation is based on growing and some of simplifying. 
...
which have ``widths'' less than $r_\mathrm{h} = 2\sqrt{a_\mathrm{h}/\pi}$.
Variable $a_\mathrm{h}$ is the least area for a hole to be presented on map.
Following \citep{Chaudhry2008}, we set $a_\mathrm{h} = 8\,\mathrm{mm}^2$.
Variable $r_\mathrm{h}$ is the radius of such a hole 
supposing that the hole is a circle.
...
We put the original building in the same group if 
