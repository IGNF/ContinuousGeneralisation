...
are used as input, 
and then maps at intermediate scales are produced 
while the start map is morphed to the goal map.
In order to morph, correspondences between two maps have to be defined.
...
straight-line trajectories are often used to interpolate.
\citet{Peng2013_LSA} investigated the problem of defining interpolate 
trajectories 
based on least squares adjustment.
Using morphing, \citet{Peng2016_Admin} continuously generalized between maps of 
...
There are many methods of generalizing buildings.
For example, \citet{haunertwolff2010} generalized a set of buildings 
without topological conflicts based an integer program. 
Their method minimizes the number of edges of all the buildings 
...
buildings become smaller and the distance between buildings become smaller too. 
In order to make sure that the map is easily perceived, 
...
The process of enlarging and aggregating can be used as a new way 
to compute urban settlement boundaries as did by
\citet{Chaudhry2008}, 
who identified the boundaries based on calculating 'citeness'.
If we have to consider enlarging, simplifying, 
and aggregating buildings simultaneously,
the continuous generalization becomes more complicated.
...
\citet{vanSmaalen2003, Stoter2009},



Our contributions are as follows.
In \sect\ref{sec:Methodology},
we generalize buildings on a start map to obtain buildings on a goal map 
by growing and merging. 
We simplify the buildings on the 
Using the start map and the goal map,
we generate maps at intermediate scales. 

a workflow based on compatible
triangulations for generalizing administrative boundaries in a continuous and 
topologically
consistent way; see Section 2.We do a thorough case study with the boundaries
of the counties and the provinces of Mainland China to test the effectiveness
and the efficiency of our method; see Section 3. We conclude in Section 4.
...
\subsection{Merging close buildings by introducing bridges}
\label{sec:Merge}
