...
As the buffers of $q$ and~$r$ overlap with each other
(see \fig\ref{fig:BridgeMoreBuilding}c),
we aggregated buildings $q$ and~$r$ by adding a bridge.
There are two buildings left in \fig\ref{fig:BridgeMoreBuilding}d.
Because of the bridge, when we grow and buffer the buildings in 
\fig\ref{fig:BridgeMoreBuilding}d, 
the buffer of building $p$ overlap with the buffer of the bridged $q$ and $r$
...
\begin{figure}[tb]
	...
	\caption{Iteratively aggregating close buildings by adding bridges.
		...
		(b) grow the original buildings with distance $\dtrm{G}$.
		(c) buffer the grown buildings with distance $d_{\epsilon,t}/2$ using 
		round joins.
		(d) aggregate buildings $q$ and~$r$ by adding a bridge,
		as their buffer overlap with each other in (c).
		(e) grow the buildings in (d) with distance $\dtrm{G}$.
		(f) buffer the grown buildings in (e) with distance $d_{\epsilon,t}/2$ 
		using round joins.
		(g) aggregate buildings $p$, $q$ and~$r$ by adding bridges.
		(h) grow the buildings in (g) with distance $\dtrm{G}$.
		(i) buffer the grown buildings in (h) with distance $d_{\epsilon,t}/2$ 
		using round joins.
	}
...
%If the buffers of two buildings overlap with each other,
...
See for example \fig\ref{fig:GrowAndBridge}, 
the building at time $t=1$ is the goal form of the three original buildings of.
...
we clip the building using the goal form 
