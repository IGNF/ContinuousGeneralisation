...
nearest points.
These line segments may cross each other or even intersect buildings.
To make the topology simple, 
we only select some of the line segments as bridges.  
...
At any time $t$, some of the identified bridges are picked to aggregate 
buildings 
...
the buffers of building $p$ and the buffer of building $q$ overlap each other, 
but the two buffers do not overlap with the buffer of building $r$.
...
\begin{figure}[tb]
	...
	\caption{Group buildings based on buffering.
		...
		using round joins, where $\alpha=2$.
