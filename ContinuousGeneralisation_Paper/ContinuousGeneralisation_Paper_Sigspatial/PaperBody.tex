...
As a result, the line segments corresponding to the edges in the 
MST
...
This merge can also avoid bridge's disappearing.
Recall that we always add a bridge linking the nearest points of 
two buildings in order to aggregate.
It can happen that 
the pair of nearest points change during growing.
\fig\ref{fig:NearestPoints} shows such an example.
For this example, at the beginning a bridge will added 
to link the pair of points marked by the squares. 
Later at some point, a bridge will be added 
to link the two points marked by the circles, 
while the first bridge will disappear.
This disappearing annoys users.
Fortunately, the merge operation makes the first bridge stay.
...
Although we iteratively aggregate (see \sect\ref{sec:Aggregate}),
it is unlikely that we need to repeat the aggregation more than 
twice.
Therefore, let us say that the iterative aggregation does not
take extra running time.
...
As a result, the simplification dominates our running time,
which is in $O(n^3)$.
...
\begin{figure*}[tb]
	...
	\caption{A sequence of maps at time~$t \in \{0, 0.1, 0.2, 
	\dots, 1\}$
		for the region marked in 
		\fig\ref{fig:ExperimentalComparison}.
	}
