...
C\# (Microsoft Visual Studio 2015) and ArcObjects SDK 10.4.1. The code is available in open source on Github \footnote{https://github.com/IGNF/Continuous-Generalisation}.
...
\emph{Stopwatch}.
Our testing data is extracted from a dataset produced the French Mapping Agency (IGN);
...
A town is defined by \citet{Boffet2000} as follows: the longest edge in an MST of the 
...
\begin{figure}[tb]
	...
	%	ylabel style={at={(axis description cs:0,1.1)},rotate=270,anchor=north},
...
%	\addplot+ [domain=0:1,samples=11] {2590* (15/(15+35* x))^1.5};
	\addplot+  coordinates {
		(0,2590)
		(0.1,1963)
		(0.2,758)
	...
	\end{axis}
	\end{tikzpicture}
	\caption{A comparison of the numbers of building between our results and 
...
%The offsetting function and clipping function are from library 
%\textsc{Clipper} 
...
%The data is at scale $1:15{,}000$, which means $M_\mathrm{s}=15{,}000$.
%It represents the buildings of four towns (communes in French), 
%i.e., Aussevielle, Denguin,  Poey-de-Lescar, and Siros, 
...
%We have unexaggerated-area symbols, 
%so for \eq2 of \citet{Topfer1966}, we used $C_\mathrm{b2}$ and $C_\mathrm{z3}$.
%As a result, we computed the number according to
%\[
%n_t=n_\mathrm{s}\frac{M_\mathrm{s}}{M_t},
%\]
%where $n_s=2{,}590$ is the number of buildings on start map
...
%According to \fig\ref{fig:BuilidngNumber}, our numbers decrease faster than 
%the numbers from the radical law.
%It seems that our result supports the fractal law proposed by 
%\citet{Jiang2015}.
%
%
%
...
%\fig\ref{fig:ExperimentalComparison} shows a comparison of 
%our result at time $t=1$
%and the data from IGN at scale $1:50{,}000$.
%No small buildings are removed in our result or the IGN data.
%The boundaries of our built-up areas are more straight than that of IGN data, 
%where we have $1{,}135$ edges while the data has $4{,}968$ edges.
%From this perspective, we would say that our result is 
%more reasonable than the existing data.
...
We compared the numbers of buildings of our results, and it is consistent with the fractak radical law from \citet{Jiang2015}.
