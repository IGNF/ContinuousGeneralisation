...
namespace MorphingClass.CAid
{
    public class CSelectRandomly : CMorphingBaseCpl
    {
        ...
        {
            Construct<CPolyline, CPolyline>(ParameterInitialize, 1, 0, false, blnCalDistanceParameters: false);
        }


        public void SelectRandomly(double dblPortion)
        {
            var iptlt = this.ObjIGeoLtLt[0].AsExpectedClass<IPoint, object>().ToList();
