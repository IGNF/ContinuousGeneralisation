...
{
    public class CPolyline : CPolyBase
    {
        ...
        public CPolyline GetSubPolyline(int frID, int toID)
        {
            ...
            var subcptlt = fcptlt.GetRange(intStartIndex, intCount);
        ...
        public CPolyline Move(double dblMoveX, double dblMoveY, double dblMoveZ = 0)
        {
            return new CPolyline(this.ID, CGeoFunc.MoveCptEb(this.CptLt, dblMoveX, dblMoveY, dblMoveZ).ToList());
        }
        ...
        public void CreateSubPllt()
        {
            ...
            var subcpllt = new List<CPolyline>(cptlt.Count - 1);
            for (int i = 0; i < cptlt.Count - 1; i++)
            {
                var subcptlt = new List<CPoint>(2);
                subcptlt.Add(cptlt[i]);
                subcptlt.Add(cptlt[i + 1]);
                var subcpl = new CPolyline(i, subcptlt);
                subcpllt.Add(subcpl);
            }
            _SubCPlLt = subcpllt;
        }
