...
{
    public static class CHelperFunctionExcel
    {
        ...
        public static void ExportEvaluationToExcelCorr(CParameterResult pParameterResult, string strPath, string strSuffix = null)
        {
            ...
            {
                string strEntireFileName = strPath + "\\" + strfilename + strEvaluationMethod + CHelperFunction.JudgeAndAddZero(i, 4) + "__" + dblSumEvaluationLtLt[i].GetLast_T().ToString();
            ...
            {
                dblSumEvaluationLt.Add(dblSumEvaluationLtLt[i].GetLast_T());
            }
        ...
        public static CParameterResult InputDataResultPtLt(string strPath)
        {
            Excel.Application pExcelAPP = new Excel.Application();
            pExcelAPP.Visible = false;
            Workbook pWorkBook = pExcelAPP.Workbooks.Open(strPath, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
        ...
        /// <summary>
        /// Input Data from Excel, Reverse
        /// </summary>
        /// <param name="strPath">文件路径</param>
        /// <returns>pParameterResult：较大比例尺线状要素上、较小比例尺线状要素、对应点列</returns>
        public static CParameterResult InputDataReverseResultPtLt(string strPath)
        {
            Excel.Application pExcelAPP = new Excel.Application();
            pExcelAPP.Visible = false;
            Workbook pWorkBook = pExcelAPP.Workbooks.Open(strPath, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
            ...
            for (int i = 0; i < intRow - 1; i++)
            {
                CPoint frcpt = new CPoint(Convert.ToInt32(values.GetValue(i + 2, 4)), Convert.ToDouble(values.GetValue(i + 2, 5)), Convert.ToDouble(values.GetValue(i + 2, 6)));
                CPoint tocpt = new CPoint(Convert.ToInt32(values.GetValue(i + 2, 1)), Convert.ToDouble(values.GetValue(i + 2, 2)), Convert.ToDouble(values.GetValue(i + 2, 3)));
