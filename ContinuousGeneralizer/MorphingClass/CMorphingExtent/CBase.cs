...
namespace MorphingClass.CMorphingExtend
{
    ...
    public class CBase
    {
        ...
        ///// <param name="dblProp">差值参数</param>
        ///// <returns>在处理面状要素时，本程序将原面状要素的边界切开，按线状要素处理，处理完后再重新生成面状要素</returns>
        //public CPolyline CGeoFunc.GetTargetcpl(double dblProp)
        //{
        //    CParameterResult pParameterResult = _DataRecords.ParameterResult;
        ...
        //        for (int j = 0; j < pCorrCptsLt[i].FrCpt.CorrespondingPtLt.Count; j++)
        //        {
        //            X0[intCount, 0] = (1 - dblProp) * pCorrCptsLt[i].FrCpt.X + dblProp * pCorrCptsLt[i].FrCpt.CorrespondingPtLt[j].X;
        //            X0[intCount + 1, 0] = (1 - dblProp) * pCorrCptsLt[i].FrCpt.Y + dblProp * pCorrCptsLt[i].FrCpt.CorrespondingPtLt[j].Y;
        //            intCount += 2;
        //        }
        //    }
        //    X0[intCount, 0] = (1 - dblProp) * dblFrLength + dblProp * dblToLength;
        //    //X0[intCount + 1, 0] = (1 - dblProp) * dblFrArea + dblProp * dblToArea;
